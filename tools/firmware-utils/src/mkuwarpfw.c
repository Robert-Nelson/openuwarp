...
static char *vendor = "PIKA Technologies";
...
static int check_options(void)
{
	...
	if (combined) {
		if (kernel_info.file_size >
			...
			ERR("kernel image is too big %d - %d", kernel_info.file_size, sizeof(struct fw_header));
			return -1;
		}
	} else {
		if (kernel_info.file_size >
		    rootfs_ofs - sizeof(struct fw_header)) {
			ERR("kernel image is too big %d - %d", kernel_info.file_size, rootfs_ofs - sizeof(struct fw_header));
			return -1;
		}
